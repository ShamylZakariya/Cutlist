...
impl Board {
    ...
    pub fn parse(spec: &str) -> Result<Board> {
        if let Some((length, width)) = spec.split_once("x") {
...
mod spec_tests {
    ...
    fn board_parse_accepts_valid_input() {
        assert_eq!(
            Board::parse("96.5x5.5").expect("Expected format to parse"),
            Board {
                length: 96.5,
                width: 5.5
            }
        ...
        assert_eq!(
            Board::parse("96x5").expect("Expected format to parse"),
            Board {
                length: 96f32,
                width: 5f32
            }
    ...
    fn board_parse_rejects_invalid_input() {
        ...
        assert!(Board::parse("-3x5.5").is_err());
        assert!(Board::parse("0x5.5").is_err());
        assert!(Board::parse("0.0x5.5").is_err());
        assert!(Board::parse("10x0").is_err());
        assert!(Board::parse("10x0.0").is_err());
        assert!(Board::parse("10x-0.01").is_err());
        assert!(Board::parse("10x-1").is_err());
